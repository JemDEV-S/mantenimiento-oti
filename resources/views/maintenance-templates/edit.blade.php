<x-layouts.app title="Editar Plantilla" moduleLabel="Mantenimiento" pageTitle="Editar Plantilla">

    <x-ui.page-header
        :title="'Editar plantilla: ' . $maintenanceTemplate->name"
        description="Modifica los textos predeterminados e ítems de la plantilla."
    >
        <x-slot:actions>
    ...
    <form method="POST" action="{{ route('maintenance-templates.update', $maintenanceTemplate) }}"
        ...
        <div class="space-y-4">
            ...
            <x-ui.card>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div class="sm:col-span-2">
                        <x-ui.input name="name" label="Nombre de la plantilla" :value="old('name', $maintenanceTemplate->name)" required />
                    </div>
                    <div>
                        ...
                        <select name="maintenance_type" class="block w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 focus:border-sigat-500 focus:ring-2 focus:ring-sigat-500/20 focus:outline-none">
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type->value }}" {{ old('maintenance_type', $maintenanceTemplate->maintenance_type?->value) === $type->value ? 'selected' : '' }}>{{ $type->label() }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <x-ui.input name="next_maintenance_interval_days" type="number" label="Intervalo siguiente mantenimiento (días)" :value="old('next_maintenance_interval_days', $maintenanceTemplate->next_maintenance_interval_days)" placeholder="Ej: 180" min="1" max="3650" />
                    </div>
                    <div class="sm:col-span-2">
                        <x-ui.textarea name="description" label="Descripción interna" :value="old('description', $maintenanceTemplate->description)" rows="2" />
                    </div>
                    <div class="sm:col-span-2 flex items-center gap-3">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                               {{ old('is_active', $maintenanceTemplate->is_active) ? 'checked' : '' }}
                               class="h-4 w-4 rounded border-slate-300 text-sigat-600 focus:ring-sigat-500">
                        <label for="is_active" class="text-sm text-slate-700">Plantilla activa</label>
                    </div>
                </div>
            </x-ui.card>
            ...
            <x-ui.card>
                ...
                <div class="space-y-4">
                    <x-ui.textarea name="problem_description" label="Descripción del problema (predeterminado)" :value="old('problem_description', $maintenanceTemplate->problem_description)" rows="2" />
                    <x-ui.textarea name="diagnosis_template" label="Diagnóstico técnico (predeterminado)" :value="old('diagnosis_template', $maintenanceTemplate->diagnosis_template)" rows="3" />
                    <x-ui.textarea name="actions_template" label="Acciones realizadas (predeterminado)" :value="old('actions_template', $maintenanceTemplate->actions_template)" rows="3" />
                </div>
            </x-ui.card>
                        ...
                        <div class="grid grid-cols-12 gap-2 items-start p-3 rounded-xl bg-slate-50">
                            ...
                            <div class="col-span-6 sm:col-span-2">
                                <input type="number" :name="`items[${i}][quantity]`" x-model="item.quantity"
                                       placeholder="Cantidad" step="0.01" min="0.01"
                                       class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-sigat-500 focus:ring-2 focus:ring-sigat-500/20 focus:outline-none">
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <input type="number" :name="`items[${i}][unit_cost]`" x-model="item.unit_cost"
                                       placeholder="Costo unit." step="0.01" min="0"
                                       class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 focus:border-sigat-500 focus:ring-2 focus:ring-sigat-500/20 focus:outline-none">
                            </div>
    ...
    function templateForm() {
        return {
            steps: @json(old('steps_json', $maintenanceTemplate->steps_json ?? [])),
            items: @json(old('items', $maintenanceTemplate->items->map(fn($i) => [
