...
<div class="space-y-1.5">
    ...
    <select
        id="{{ $name }}"
        name="{{ $name }}"
        {{ $attributes->merge(['class' => 'block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 transition-colors focus:border-sigat-500 focus:ring-2 focus:ring-sigat-500/20 focus:outline-none disabled:bg-slate-50 appearance-none']) }}
    >
